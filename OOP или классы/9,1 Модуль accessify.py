...
# Декоратор @protected позволяет сделать метод защищенным, то есть доступным только внутри класса и его наследников,
# в отличии от обычного защищённого метода с одним подчёркиванием, который можно вызвать извне.
...
# Здесь мы использовали декоратор @protected, чтобы сделать метод _protected защищенным. В примере мы видим,
# что защищенный метод _protected, не может быть вызван напрямую, но может быть вызывать через публичный метод _public.
